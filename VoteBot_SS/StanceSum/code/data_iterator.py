...
class Dataset(object):
    ...
    def _parse_file2doc_bert(self, file_name):
        ...
        with open(file_name, 'r') as f:
            obj = json.load(f)
            for j_str in obj['dataset']:
                article, abstract = j_str['doc'], j_str['target']
                tokenized_article = [self._tokenizer.tokenize(sen) for sen in article]

                issue_sim = j_str['issue_sim']
                intent_prob = j_str['prob']
                ...
                yield article_token_ids, article_seg_ids, article_token_ids_c, article_seg_ids_c, pair_indice, article, abstract, issue_sim, intent_prob

    def _doc_iterate_bert(self, docs):

        for article_token_ids, article_seg_ids, article_token_ids_c, article_seg_ids_c, pair_indice, article, abstract, issue_sim, intent_prob in docs:
            issue_sim = np.array(issue_sim).reshape((-1, 1))
            if len(article_token_ids) == 0:
                yield article, abstract, issue_sim, intent_prob, (None, None, None, None, None, None, None)
            ...
            yield article, abstract, issue_sim, intent_prob, (out_x, out_t, out_w, out_x_c, out_t_c, out_w_c, pair_indice)
